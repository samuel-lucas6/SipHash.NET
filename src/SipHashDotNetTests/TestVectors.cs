...
namespace SipHashDotNetTests;

[TestClass]
public class TestVectors
{
    // https://github.com/veorq/SipHash/blob/master/vectors.h
    private static readonly byte[][] ExpectedTags =
    {
    ...
    {
        Span<byte> key = stackalloc byte[] { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
        var message = new List<byte>();
        Span<byte> tag = stackalloc byte[SipHash.TagSize];
        for (int i = 0; i < ExpectedTags.Length; i++) {
            Span<byte> messageBytes = message.ToArray();
            SipHash.ComputeTag(tag, messageBytes, key);
            Assert.IsTrue(tag.SequenceEqual(ExpectedTags[i]));
            bool valid = SipHash.VerifyTag(tag, messageBytes, key);
            Assert.IsTrue(valid);
            message.Add((byte)i);
        }
    }
